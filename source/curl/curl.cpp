...
void curl::enable_tls_verification(curl::Handle &curl)
{
    static constexpr const char *CA_BUNDLE = "romfs:/certs/cacert.pem";

    curl::set_option(curl, CURLOPT_SSL_VERIFYHOST, 2L);
    curl::set_option(curl, CURLOPT_SSL_VERIFYPEER, 1L);
    curl::set_option(curl, CURLOPT_CAINFO, CA_BUNDLE);
}
