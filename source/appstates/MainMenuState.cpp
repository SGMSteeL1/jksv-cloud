...
void MainMenuState::confirm_update()
{
    ...
    std::string query{confirmUpdate};
    if (!m_dataStruct->updateVersion.empty())
    {
        query.append("\n\nJKSV Cloud ");
        query.append(m_dataStruct->updateVersion);
    }

    ConfirmProgress::create_push_fade(query, false, tasks::update::download_update, nullptr, m_dataStruct);
}
