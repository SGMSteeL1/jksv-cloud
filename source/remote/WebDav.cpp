    ...
    : Storage("[WD]")
{
    ...
    WebDav::initialize_listing();
}

remote::WebDav::WebDav(const WebDavConfig &config)
    : Storage(config.prefix, config.supportsUtf8)
    , m_origin(config.origin)
    , m_username(config.username)
    , m_password(config.password)
    , m_verifyTls(config.verifyTls)
{
    if (m_origin.empty() || config.basePath.empty()) { return; }

    m_root   = stringutil::get_formatted_string("/%s/", config.basePath.c_str());
    m_parent = m_root;
    WebDav::initialize_listing();
}
...
remote::WebDav::~WebDav()
{
    ...
    std::fill(m_username.begin(), m_username.end(), '\0');
}
...
bool remote::WebDav::upload_file(const fslib::Path &source, std::string_view remoteName, sys::ProgressTask *task)
{
    ...
    {
        logger::log(STRING_ERROR_UPLOADING, "Unexpected HTTP response!");
        return false;
    }
...
bool remote::WebDav::patch_file(remote::Item *item, const fslib::Path &source, sys::ProgressTask *task)
{
    ...
    if (code != 200 && code != 201 && code != 204)
    {
        logger::log(STRING_ERROR_PATCHING, "Unexpected HTTP response!");
        return false;
    }
...
{
    if (m_verifyTls) { curl::enable_tls_verification(m_curl); }
    curl::set_option(m_curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
...
void remote::WebDav::initialize_listing()
{
    remote::URL url{m_origin};
...
bool remote::WebDav::prop_find(const remote::URL &url, std::string &xml)
{
    ...
    return curl::perform(m_curl) && curl::get_response_code(m_curl) == 207;
}
