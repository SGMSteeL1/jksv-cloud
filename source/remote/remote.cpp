...
namespace
{
    ...
    constexpr const char *STRING_JKSV_DIR = "JKSV Cloud";
...
static bool initialize_nextcloud();
...
{
    const bool nextcloudExists = remote::nextcloud_is_configured();
    ...
    if ((nextcloudExists || driveExists || webdavExists) && !remote::has_internet_connection())
    {
    ...
    if (nextcloudExists)
    {
        ...
        const bool initialized = initialize_nextcloud();
        const char *message = strings::get_by_name(strings::names::NEXTCLOUD,
                                                    initialized ? 5 : 6);
        if (message) { ui::PopMessageManager::push_message(popTicks, message); }
    }
    else if (driveExists) { initialize_google_drive(); }
...
static bool initialize_nextcloud()
{
    ...
    const remote::NextcloudResult loaded = remote::nextcloud_load_credentials(credentials);
    if (loaded != remote::NextcloudResult::Ok)
    {
        logger::log("Nextcloud initialization failed: %s", remote::get_nextcloud_result_string(loaded));
        return false;
    }

    remote::WebDavConfig config{
        .origin       = credentials.server,
    ...
    auto storage = std::make_unique<remote::WebDav>(config);
    std::fill(credentials.appPassword.begin(), credentials.appPassword.end(), '\0');
    std::fill(config.password.begin(), config.password.end(), '\0');
    if (!storage->is_initialized()) { return false; }
...
bool remote::reload_nextcloud() noexcept { return initialize_nextcloud(); }

bool remote::disconnect_nextcloud() noexcept
{
    if (!remote::nextcloud_is_configured()) { return true; }

    remote::NextcloudCredentials credentials{};
    if (remote::nextcloud_load_credentials(credentials) == remote::NextcloudResult::Ok)
    {
        if (!remote::nextcloud_revoke_credentials(credentials))
        {
            logger::log("Nextcloud app-password revocation failed; removing the local vault anyway.");
        }
        std::fill(credentials.appPassword.begin(), credentials.appPassword.end(), '\0');
    }

    s_storage.reset();
    return remote::nextcloud_delete_credentials();
}

bool remote::nextcloud_is_configured() noexcept
{
    return fslib::file_exists(fslib::Path{remote::PATH_NEXTCLOUD_VAULT});
}
